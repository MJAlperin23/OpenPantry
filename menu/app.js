...
function updateMessage(res, input, data) {
  if(makeing_food(data)){
    ...
    if(data.entities[1]) {
      var path = getCityURL(data.entities[1].value);
    } else {
      ...
      resp.on('end', function(){
        ...
        if(chunkJSON.location) {
          ...
          if ( when == 'today' ) {
            ...
            forecast = chunkJSON.forecast.txt_forecast.forecastday[4].fcttext;
          } else{
            forecast = chunkJSON.forecast.txt_forecast.forecastday[0].fcttext;
          }
...
function makeing_food(data){
  ...
  return false;
  // return data.intents && data.intents.length > 0 && data.intents[0].intent === 'Meals_to_make'
